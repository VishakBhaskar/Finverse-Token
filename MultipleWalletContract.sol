...
contract Finverse is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    // uint256 public _feeDecimal = 0; // Decimals due to high total supply
    ...
    bool private inSwap; //lock the swap modifier
    bool public swapEnabled = true;

    uint256 public maxTxAmount = _tokenTotal.mul(10).div(1000); // 1% OF TOTAL TOKEN SUPPLY 
    uint256 public minTokensBeforeSwap = 100 * 10**18; // Total Fee should be greater then this
    ...
    function swap() private lockTheSwap {
        ...
        // uint256 totalFee = _liqFeeCollected.add(_developmentFeeCollected);

        if(minTokensBeforeSwap > totalFee) return;

        uint256 amountToLiquify = totalFee.mul(_liqFeeCollected).div(totalFee).div(2);
        uint256 feeSub = amountToLiquify.add(_burnFeeCollected); // Fee not swaped in BNB
        ...
        // uint256 amountBNBBurn = amountBNB.mul(_burnFeeCollected).div(totalBNBFee);
        ...
        // if(amountBNBBurn > 0) payable(burnWallet).transfer(amountBNBBurn);
